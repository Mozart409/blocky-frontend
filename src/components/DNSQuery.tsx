...
const isValidIPv6 = (ip: string): boolean => {
  const ipv6Pattern =
    /^(?:[a-fA-F0-9]{1,4}:){7}[a-fA-F0-9]{1,4}$|^::(?:[a-fA-F0-9]{1,4}:){0,6}[a-fA-F0-9]{1,4}$|^(?:[a-fA-F0-9]{1,4}:){1,7}:$|^(?:[a-fA-F0-9]{1,4}:){0,6}::(?:[a-fA-F0-9]{1,4}:){0,5}[a-fA-F0-9]{1,4}$/
      ...
      >
        <div className="mb-4">
          <label className="block text-gray-700 text-sm font-bold mb-2">
            <span>
              {type === 'PTR' ? 'Enter an IP address' : 'Enter a domain name'}
            </span>
            <input
              required
              value={domain}
